...
Data rules (PLAN.md / replication guide / DASHBOARD_SPEC.md):
  ...
  * Trading days = weekdays minus SA public holidays (sa_calendar.py; Larry,
    13 Aug 2026). The workbook's day-count cells stay editable assumptions.
"""
...
from sa_calendar import nth_trading_day, trading_days_between
from style import (
    ALT,
    BLUE,
    DEC2,
...
def month_trading_days(fy: int, m: int) -> int:
    ...
    Changed 13 Aug 2026 on Larry's instruction ("Monday was a public holiday
    so we should be on day 6" — re the Aug FY27 projection): public holidays
    come OUT of the count. The previous holidays-left-in reading came from his
    28 Jul reference workbook, but that reference only ever showed completed
    months, where elapsed = period and the choice cancels; August 2026 was the
    first live test. Gives Aug 20 (not 21), Apr 19, May 20, Jun 21; Mar 22 and
    Jul 23 are unchanged (no weekday holidays). See sa_calendar.py for the
    holiday rules and the EXTRA_HOLIDAYS / EXTRA_WORKDAYS exception lists.
    """
...
class Budget:
    def __init__(self, path):
        rows = (
            ...
            .get_sheet_by_name("Client Monthly Compare")
            .to_python(skip_empty_area=False)
        )
        hdr = [str(h).strip() for h in rows[4]]
        assert hdr[3] == "Acct" and hdr[5].startswith("Mar"), (
            f"Budget layout changed: {hdr[:8]}"
        )
...
def load_pools(
    ...
    for r in rows:
        ...
        if d is None:
            continue
        inv.add(
            fold(acct_str(r[ix["Account"]])),
            d,
            r[ix["Subtotal"]] or 0,
            r[ix["Chrg Mass"]] or 0,
            str(r[ix["Customer"]]).strip(),
    ...
    for r in rows3:
        ...
        if d is None:
            continue
        wbp.add(
            fold(acct_str(r[ix3["Account"]])),
            d,
            r[ix3["Subtotal"]] or 0,
            r[ix3["Chrg Mass"]] or 0,
            str(r[ix3["Customer"]]).strip(),
...
class Model:
    def __init__(
        self, inv: Pool, py: Pool, wbp: Pool, credits: Credits, budget: Budget
    ):
        self.inv, self.py, self.wbp, self.credits, self.budget = (
            inv,
            py,
            wbp,
            credits,
...
def build_tab1(wb, st, M: Model, p):
    ...
    # table header
    th = F(
        st, bold=True, font_size=9, font_color="white", bg_color=NAVY, align="center"
    )
    th2 = F(
        st, bold=True, font_size=9, font_color=YELLOW, bg_color=NAVY2, align="center"
    )
    tho = F(
        st, bold=True, font_size=9, font_color="white", bg_color=ORANGE, align="center"
    )
    ...
    for j, (m, lab) in enumerate(months):
        ...
        write_month_row(
            r,
            lab,
            rowvals(month_win(cal_year(FY, m), m), month_win(cal_year(FY, m) - 1, m)),
            F(st, font_size=10, bg_color=bg),
            bg,
        )
    ...
    ytd = month_derived(
        *[sum(rowv[rr][cl] for rr in mrows) for cl in (2, 3, 5, 6, 10, 11)],
        guarded=False,
    )
    rowv[R_YTD] = ytd
    ...
    for cl, letter in [(2, "C"), (3, "D"), (5, "F"), (6, "G"), (10, "K"), (11, "L")]:
        V.f(
            r - 1,
            cl,
            f"=SUM({letter}14:{letter}{R_YTD - 1})",
    ...
    V.f(
        r - 1,
        12,
        f'=IF(L{R_YTD}=0,"",K{R_YTD}/L{R_YTD}-1)',
    ...
    V.f(
        r - 1,
        13,
        f'=IF(K{R_YTD}=0,"",E{R_YTD}/K{R_YTD})',
        F(st, num_format=DEC2, **ob),
        value=ytd[13],
    )
    V.f(
        r - 1,
        14,
        f'=IF(L{R_YTD}=0,"",H{R_YTD}/L{R_YTD})',
        F(st, num_format=DEC2, **ob),
        value=ytd[14],
    )
    V.f(
        r - 1,
        15,
        f'=IF(OR(K{R_YTD}=0,L{R_YTD}=0,H{R_YTD}=0),"",(E{R_YTD}/K{R_YTD})/(H{R_YTD}/L{R_YTD})-1)',
        F(st, num_format=PCT1, **ob),
        value=ytd[15],
    )
    ...
    if has_mtd:
        ...
        ws.write(r - 1, 3, c, F(st, num_format="(#,##0)", **blue))
        V.f(
            r - 1, 4, f"=C{R_MTD}-D{R_MTD}", F(st, num_format=NUM, **base), value=jul[4]
        )
        ws.write(r - 1, 5, gp, F(st, num_format=NUM, **blue))
        ws.write(r - 1, 6, cp, F(st, num_format="(#,##0)", **blue))
        V.f(
            r - 1, 7, f"=F{R_MTD}-G{R_MTD}", F(st, num_format=NUM, **base), value=jul[7]
        )
        V.f(
            r - 1,
            8,
        ...
        V.f(
            r - 1,
            12,
            f'=IF(L{R_MTD}=0,"",K{R_MTD}/L{R_MTD}-1)',
            F(st, num_format=PCT1, **base),
            value=jul[12],
        )
        V.f(
            r - 1,
            13,
            f'=IF(K{R_MTD}=0,"",E{R_MTD}/K{R_MTD})',
            F(st, num_format=DEC2, **base),
            value=jul[13],
        )
        V.f(
            r - 1,
            14,
            f'=IF(L{R_MTD}=0,"",H{R_MTD}/L{R_MTD})',
            F(st, num_format=DEC2, **base),
            value=jul[14],
        )
        V.f(
            r - 1,
            15,
            f'=IF(OR(K{R_MTD}=0,L{R_MTD}=0,H{R_MTD}=0),"",(E{R_MTD}/K{R_MTD})/(H{R_MTD}/L{R_MTD})-1)',
            F(st, num_format=PCT1, **base),
            value=jul[15],
        )
        ...
        r = R_INC
        incl = month_derived(
            *[rowv[R_YTD][cl] + rowv[R_MTD][cl] for cl in (2, 3, 5, 6, 10, 11)],
            guarded=False,
        )
        rowv[R_INC] = incl
        ...
        V.f(
            ...
            8,
            f"=E{R_INC}-H{R_INC}",
            F(st, num_format="#,##0;(#,##0)", **yb),
            value=incl[8],
        )
        V.f(
            r - 1, 9, f"=I{R_INC}/H{R_INC}", F(st, num_format=PCT1, **yb), value=incl[9]
        )
        V.f(
            r - 1,
            12,
            f'=IF(L{R_INC}=0,"",K{R_INC}/L{R_INC}-1)',
        ...
        V.f(
            r - 1,
            13,
            f'=IF(K{R_INC}=0,"",E{R_INC}/K{R_INC})',
            F(st, num_format=DEC2, **yb),
            value=incl[13],
        )
        V.f(
            r - 1,
            14,
            f'=IF(L{R_INC}=0,"",H{R_INC}/L{R_INC})',
            F(st, num_format=DEC2, **yb),
            value=incl[14],
        )
        V.f(
            r - 1,
            15,
            f'=IF(OR(K{R_INC}=0,L{R_INC}=0,H{R_INC}=0),"",(E{R_INC}/K{R_INC})/(H{R_INC}/L{R_INC})-1)',
            F(st, num_format=PCT1, **yb),
            value=incl[15],
        )
    ...
    ws.merge_range(
        R_SEC - 1, 1, R_SEC - 1, 4, f"{cur_name.upper()} MOMENTUM (MTD, net)", sect
    )
    ws.merge_range(R_SEC - 1, 6, R_SEC - 1, 9, "CREDIT NOTES % OF GROSS", sect)
    jul_full_py = round(
        M.py.total(*month_win(cur_y - 1, cur))[0]
    ...
    pairs = [
        ...
        (
            f"Projected full {cur_name} {fyl} (net)",
            f"=E{R_P0 + 2}*E{R_P0 + 1}",
            False,
            NUM,
            e28,
        ...
        (
            f"Projected {cur_name} growth vs PY",
            f"=E{R_P0 + 5}/E{R_P0 + 6}-1",
            False,
            PCT1,
            e30,
        ),
    ...
    right = [
        (
            f"{fyl} (YTD {span})",
            f"=D{R_YTD}/C{R_YTD}",
            div(rowv[R_YTD][3], rowv[R_YTD][2], blank=0),
        ),
        (
            f"{ly} (YTD {span})",
            f"=G{R_YTD}/F{R_YTD}",
            div(rowv[R_YTD][6], rowv[R_YTD][5], blank=0),
        ),
        (
            f"{fyl} (incl. {cur_ab} MTD)",
            f"=D{R_INC}/C{R_INC}",
            div(rowv[R_INC][3], rowv[R_INC][2], blank=0),
        ),
        (
            f"{ly} (incl. {cur_ab} MTD)",
            f"=G{R_INC}/F{R_INC}",
            div(rowv[R_INC][6], rowv[R_INC][5], blank=0),
        ),
    ]
...
def customer_universe(M: Model, w27, wly, months, memo_mode="unified"):
    ...
    for a, c in M.cls.items():
        ...
        if a in M.budget.nondedi:
            ...
            per_rep["CLOSED"].append(
                a
            )  # LY-only listing (LY net ≠ 0, incl. credit-only)
    ...
    def rep_sort(accts):
        ...
        zero.sort(
            key=lambda a: (-size(a), M.budget.rank(a), M.inv.first_seen.get(a, 10**9))
        )
...
def build_customer_tab(
    ...
    for i, w in enumerate(
        [2, 9, 34, 6, 13, 13, 12, 13, 9, 13, 13, 10, 9, 12, 12, 9, 8, 8, 9]
    ):
        ws.set_column(i, i, w)
    ...
    ws.merge_range(
        "B7:D7", "Trading days elapsed:", F(st, bold=True, font_color=NAVY, **ye)
    )
    ws.write("E7", elapsed, F(st, font_color=BLUE, bold=True, num_format="0", **ye))
    ws.merge_range(
        "F7:G7", "Trading days in period:", F(st, bold=True, font_color=NAVY, **ye)
    )
    ...
    ws.merge_range(
        "B8:S8",
        "KPIs cover the rep-allocated selling book. Non-budget accounts "
        "appear under their rep with 0 target.",
        F(st, font_size=8, font_color="#595959", border=0),
    )
    ...
    def mk_fmt(bg, bold=False, fc=None, size=10, align="right"):
        ...
        return {
            "txt": F(st, align="left", **base),
            "num": F(st, align=align, num_format=NUM, **base),
    ...
    def write_acct_row(r, a, memo, stripe):
        ...
        br = M.branch(a)
        ws.write(r - 1, 3, br, F(st, **ctr)) if br else ws.write_blank(
            r - 1, 3, None, F(st, **ctr)
        )
        ...
        V.f(
            ...
            10,
            f'=IF(F{r}=0,"",J{r}-F{r})',
            F(st, num_format=NUMP, **base),
            value=d[10],
        )
        V.f(
            r - 1,
            11,
            f'=IF(F{r}=0,"",J{r}/F{r}-1)',
            F(st, num_format=PCT1, **base),
            value=d[11],
        )
        V.f(
    ...
    for c, hrow, first, last, srow in sections:
        ws.merge_range(
            ...
            hrow - 1,
            18,
            f"{REP_DISPLAY[c]}   ({len(per_rep[c])} customers)",
            rep_hdr_fmt,
        )
    ...
    ws.merge_range(
        ...
        sb_row - 1,
        3,
        "SUBTOTAL — Rep-allocated selling book",
        sb_fmt["txt"],
    )
    ...
    for cl, letter in [(4, "E"), (5, "F"), (7, "H"), (9, "J"), (13, "N"), (14, "O")]:
        V.f(
            sb_row - 1,
            cl,
            "=" + "+".join(f"{letter}{sr}" for sr in subrefs),
            sb_fmt["num"],
            value=sbd[cl],
        )
    ...
    V.f(
        sb_row - 1,
        15,
        f'=IF(O{sb_row}=0,"",(N{sb_row}/$E$7*$H$7)/O{sb_row}-1)',
        sb_fmt["pct"],
        value=sbd[15],
    )
    V.f(
        sb_row - 1,
        16,
        f'=IF(N{sb_row}=0,"",H{sb_row}/N{sb_row})',
        sb_fmt["dec"],
        value=sbd[16],
    )
    V.f(
        sb_row - 1,
        17,
        f'=IF(O{sb_row}=0,"",E{sb_row}/O{sb_row})',
        sb_fmt["dec"],
        value=sbd[17],
    )
    V.f(
    ...
    for j, a in enumerate(house):
        write_acct_row(
            house_hdr + 1 + j, a, memo=True, stripe=(house_hdr + 1 + j) % 2 == 0
        )
    write_subtotal(
        house_sub,
        "House accounts (zeroed) subtotal",
        house_hdr + 1,
        house_sub - 1,
        memo=True,
        fmt=sub_fmt,
    ...
    for j, a in enumerate(closed):
        write_acct_row(
            closed_hdr + 1 + j, a, memo=True, stripe=(closed_hdr + 1 + j) % 2 == 0
        )
    write_subtotal(
        closed_sub,
        "Closed / lost accounts subtotal",
        closed_hdr + 1,
        closed_sub - 1,
        memo=True,
        fmt=sub_fmt,
    ...
    ws.merge_range(
        total_row - 1, 1, total_row - 1, 3, "TOTAL — ALL ACCOUNTS", tot_fmt["txt"]
    )
    td = agg[total_row]
    for cl, letter in [(4, "E"), (5, "F"), (7, "H"), (9, "J"), (13, "N"), (14, "O")]:
        V.f(
    ...
    V.f(
        total_row - 1, 11, f"=J{total_row}/F{total_row}-1", tot_fmt["pct"], value=td[11]
    )
    V.f(
        total_row - 1, 12, f"=J{total_row}/E{total_row}-1", tot_fmt["pct"], value=td[12]
    )
    V.f(
        total_row - 1,
        15,
        f'=IF(O{total_row}=0,"",(N{total_row}/$E$7*$H$7)/O{total_row}-1)',
        tot_fmt["pct"],
        value=td[15],
    )
    V.f(
        total_row - 1,
        16,
        f'=IF(N{total_row}=0,"",H{total_row}/N{total_row})',
        tot_fmt["dec"],
        value=td[16],
    )
    V.f(
        total_row - 1,
        17,
        f'=IF(O{total_row}=0,"",E{total_row}/O{total_row})',
        tot_fmt["dec"],
        value=td[17],
    )
    V.f(
...
def build_daily_tab(wb, st, M: Model, sheet, basis, day, pool: Pool, subtract_credits):
    ...
    house_b = (
        sorted(per_rep["HOUSE"][0], key=lambda a: (-billed[a][0], M.budget.rank(a)))
        if "HOUSE" in per_rep
        else []
    )
    closed_b = (
        sorted(per_rep["CLOSED"][0], key=lambda a: (-billed[a][0], M.budget.rank(a)))
        if "CLOSED" in per_rep
        else []
    )
    ...
    ws.merge_range(
        "F7:J7",
        "Day target = monthly budget target ÷ trading days (editable)",
        F(st, font_size=8, font_color="#595959", border=0),
    )
    ...
    kpi_spec = [
        ...
        (
            ...
            "J",
            "R/kg",
            f'=IF(I{total_row}=0,"",E{total_row}/I{total_row})',
            9,
            kl_or,
            kv(ORANGE, NAVY, DEC2),
        ),
    ...
    for c, hrow, first, last, srow in sections:
        ...
        ws.merge_range(
            ...
            hrow - 1,
            9,
            f"{REP_DISPLAY[c]}   ({len(b)} billed, {len(nb)} not billed)",
            rep_hdr_fmt,
        )
        r = first
        for a in b:
    ...
    ws.merge_range(
        ...
        sb_row - 1,
        3,
        "SUBTOTAL — Rep-allocated (billed today)",
        F(st, **sbf),
    )
    subrefs = [s[4] for s in sections]
    ...
    V.f(
        ...
        7,
        f"=E{sb_row}/F{sb_row}",
        F(st, num_format=PCT1, **sbf),
        value=sbd[7],
    )
    V.f(
        sb_row - 1,
        9,
        f'=IF(I{sb_row}=0,"",E{sb_row}/I{sb_row})',
    ...
    for blk, hdr_r, sub_r, label in [
        ...
        ws.merge_range(
            ...
            hdr_r - 1,
            9,
            f"{label}   ({len(blk)} billed) — no target",
            memo_hdr,
        )
        r = hdr_r + 1
        for a in blk:
    ...
    ws.merge_range(
        total_row - 1, 1, total_row - 1, 3, "TOTAL — ALL ACCOUNTS BILLED", F(st, **tf)
    )
...
def build_credit_tab(wb, st, M: Model, mtd_max: date):
    ...
    for j, (rs, (val, cnt)) in enumerate(ordered):
        ...
        ws.merge_range(
            ...
            7,
            round(val),
            F(st, num_format=NUM, font_color=BLUE, **base),
        )
    ...
    V.mf(
        f"G{total_r}:H{total_r}",
        f"=SUM(G{r0}:G{total_r - 1})",
...
def build(
    ...
    build_customer_tab(
        ...
        elapsed,
        cur_days,
        (date(cur_y, cur, 1), mtd_max),
        month_win(cur_y - 1, cur),
        [cur],
        round_vals=True,
        memo_mode="mtd",
    )
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--wb-asof", default=None, help="Cap waybill-date data (YYYY-MM-DD)"
    )
    a = ap.parse_args()
    print(
        build(

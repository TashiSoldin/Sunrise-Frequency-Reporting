...
def load_day(inv_file: str, day: date):
    ...
    day_rows = [
        r
        for r in rows
        if r[ix["Invoice Date"]] == day and "~" not in str(r[ix["Waybill"]])
    ]
...
def build(
    ...
    for acc, lines in ordered:
        ...
        for line in lines:
            ...
            vals = [
                str(line[ix["Waybill"]]),
                line[ix["Waybill Date"]].strftime("%Y/%m/%d")
                if isinstance(line[ix["Waybill Date"]], date)
                else "",
                intstr(line[ix["Invoice"]]),
                acc,
                custname,
                str(line[ix["Shipper"]] or ""),
                str(line[ix["Consignee"]] or ""),
                str(line[ix["Orig Hub"]] or ""),
                str(line[ix["Dest Hub"]] or ""),
                str(line[ix["Dest Place"]] or ""),
                str(line[ix["Reference"]] or ""),
                str(line[ix["Service"]] or ""),
                line[ix["Pieces"]] or 0,
                round(line[ix["Actual Mass"]] or 0, 1),
                round(line[ix["Chrg Mass"]] or 0, 1),
                b_,
                o_,
                s_,
                f_,
            ...
            nums = [
                line[ix["Pieces"]] or 0,
                line[ix["Actual Mass"]] or 0,
                line[ix["Chrg Mass"]] or 0,
                b_,
                o_,
                s_,
                f_,
        ...
        def totals_row(label, tvals, bg):
            ...
            for j, v in enumerate(tvals):  # cols N..V = 13..21
                ws.write(
                    r_,
                    13 + j,
                    round(v, 1) if j in (1, 2) else v,
                    cellfmt(12 + j, bg, True),
                )
            ws.write(
                r_, 22, (tvals[8] / tvals[2]) if tvals[2] else 0, cellfmt(21, bg, True)
            )
    ...
    for j, v in enumerate(gtot):
        ws.write(
            r_, 13 + j, round(v, 1) if j in (1, 2) else v, cellfmt(12 + j, ORANGE, True)
        )
    ...
    for r in day_rows:
        ...
        for j, v in enumerate(
            [r[ix["Pieces"]] or 0, r[ix["Chrg Mass"]] or 0, b_, o_, s_, f_, su_, sub_]
        ):
            cust[k][j] += v
    ranked = sorted(
        ((k, cnames[k], v) for k, v in cust.items()), key=lambda kv: -kv[2][7]
    )
    title_block(
        ws3,
        st,
    ...
    for acc, cname, v in ranked:
        ...
        for j, x in enumerate(row):
            kw = {"font_size": 9, "bg_color": bg, "align": "left" if j < 2 else "right"}
            if fmts3[j]:
...
def _check_flash_parse(flash_file, f_rev, f_branch, f_rep):
    ...
    for label, parsed in (("BY BRANCH (origin)", f_branch), ("BY REP", f_rep)):
        ...
        if not parsed:
            print(
                f"WARNING: flash comparison read no rows from {label!r} in "
                f"{os.path.basename(flash_file)} — the section header has "
                f"probably been renamed.",
                file=sys.stderr,
            )
        elif f_rev and abs(total - f_rev) > 1:
            print(
                f"WARNING: flash comparison read {len(parsed)} {label!r} rows "
...
def _flash_comparison(
    wb, st, day, day_rows, ix, flash_file, long_date, inv_total, inv_kg, inv_lines
):
    ...
    rep_names = {
        str(r[ix["Salesrep"]]): str(r[ix["Rep"]]) for r in day_rows if r[ix["Rep"]]
    }
    ...
    def block(r0, title_, rows_, total=None):
        ...
        for i, (lab, a, b, wantvar, emphas) in enumerate(rows_):
            ...
            ws.write(
                rr, 2, a, st.get(num_format=nf, font_color=BLUE, align="right", **base)
            )
            ws.write(
                rr, 3, b, st.get(num_format=nf, font_color=BLUE, align="right", **base)
            )
            if wantvar:
                var = a - b
                ...
                ws.write(
                    ...
                    st.get(
                        num_format=NUMP,
                        font_color=(GREEN if var >= 0 else RED),
                        align="right",
                        **base,
                    ),
                )
                ws.write(
                    ...
                    vpct,
                    st.get(
                        num_format=PCT_SIGNED,
                        font_color=(GREEN if vpct >= 0 else RED),
                        align="right",
                        **base,
                    ),
                )
    ...
    rr = block(
        ...
        [
            ...
            (
                "R / kg",
                round(inv_total / inv_kg, 2) if inv_kg else 0.0,
                round(f_rev / f_kg, 2) if f_kg else 0.0,
                False,
                False,
            ),
    ...
    rows_ = [
        (b, round(inv_branch.get(b, 0)), round(f_branch.get(b, 0) or 0), True, False)
        for b in branches
    ]
    ...
    rows_ = [
        (
            f"{c} — {rep_names.get(c, c)}",
            round(inv_rep.get(c, 0)),
            round(f_rep.get(c, 0) or 0),
            True,
            False,
        )
        for c in BUDGET_REPS
    ]
    ...
    ws.merge_range(
        ...
        f"and some of the day's invoices cover earlier waybills.",
        note,
    )
    ws.merge_range(
        rr + 2,
        ...
        "waybills are captured — so the true shipping-vs-billing gap may be wider than shown here.",
        note,
    )
    ws.merge_range(
        rr + 3,
        ...
        "while invoices group by billing account.",
        note,
    )
...
def _consolidations(wb, st, th, day_rows, ix, short_date):
    ...
    cons = {
        fr: g
        for fr, g in groups.items()
        if any(str(x[ix["Consolidated"]]) == "Yes" for x in g)
    }
    r0_first = sorted(
        cons.items(),
        key=lambda kv: (sum((x[ix["Subtotal"]] or 0) for x in kv[1]) != 0,),
    )
    ...
    for i, w in enumerate([2, 14, 6, 8, 26, 24, 6, 6, 6, 6, 9, 11, 8]):
        ws.set_column(i, i, w)
    n_r0 = sum(
        1 for _, g in cons.items() if sum((x[ix["Subtotal"]] or 0) for x in g) == 0
    )
    title_block(
        ws,
        st,
    ...
    for fr, g in r0_first:
        ...
        ws.merge_range(
            rr, 1, rr, 12, f"Ref {fr}   —   {len(g)} waybills   —   {tag}", hstyle
        )
        rr += 1
        kg_t = pcs_t = 0.0
        for i, x in enumerate(g):
            ...
            vals = [
                str(x[ix["Waybill"]]),
                str(x[ix["Consolidated"]] or "No"),
                str(x[ix["Account"]]),
                str(x[ix["Customer"]]),
                str(x[ix["Consignee"]] or ""),
                str(x[ix["Orig Hub"]] or ""),
                str(x[ix["Dest Hub"]] or ""),
                str(x[ix["Service"]] or ""),
                x[ix["Pieces"]] or 0,
                kg,
                sub,
...
def _credit_notes(wb, st, day, credits_file, dm, short_date):
    ...
    notes = [
        r for r in rows if str(r[ic["Type"]]) in CREDIT_TYPES and r[ic["Date"]] == day
    ]
    ...
    ws.merge_range(
        "B7:I7",
        "BY REASON",
        st.get(bold=True, font_size=11, font_color="white", bg_color=NAVY),
    )
    rr = 7
    ...
    for r in notes:
        ...
        rcpt = r[ic["Receipt"]]
        vals = [
            str(int(rcpt)) if isinstance(rcpt, float) else str(rcpt),
            str(r[ic["Account"]]),
            str(r[ic["Customer Name"]]),
            str(r[ic["Rep"]]),
            str(r[ic["Branch"]]),
            str(r[ic["Reason"]]),
            r[ic["Subtotal"]] or 0,
    ...
    ws.write(
        rr, 7, total, st.get(bold=True, font_size=9, bg_color=ORANGE, num_format=NUM2)
    )
